...
bool askOkCancelCustomUserSystemPopupCommand(const std::string strTitle, const std::string strCustomMessage, const std::string strDetails, const std::string strFileToEdit, size_t lineAtFileToEdit) {
	static std::string systemPopupCmd = [](){return platform::getEnvironmentVariableValueString(systemPopupCmd, "ARX_ScriptErrorPopupCommand", Logger::LogLevel::Warning, "Attention: custom user command!").getString();}(); // being static logs only once
	...
	if(systemPopupCmd.size() && now >= ignoreTo) {
		...
		static std::string codeEditorCmd = [](){return platform::getEnvironmentVariableValueString(codeEditorCmd, "ARX_ScriptCodeEditorCommand", Logger::LogLevel::Warning, "Attention: custom user command!").getString();}();  // being static logs only once
		if(codeEditorCmd.size()) {
		...
		if(retPopupCmd == 0 && codeEditorCmd.size() && strFileToEdit.size()) { // clicked ok
