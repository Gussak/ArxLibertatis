...
enum LODFlag {
	...
	LOD_ICON    = 1 << 6, // TODO AUTOMATIC: flat, 2 triangles like 1 rectangular face, uses the item icon as texture. The mod developers should not need to create this, it shall be a simplified LOD auto generated by the engine by stretching that mesh to match the icon size ex.: mesh rect size = 1.fx1.f * iconSize = 32x64 (torch), but checks the torch model and detects width is bigger than height, then mesh rect becomes 64x32.
};
