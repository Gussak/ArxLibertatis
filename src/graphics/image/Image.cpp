...
bool Image::load(const char * data, size_t size, const char * file) {
	...
	static s32 iMax = [](){return platform::getEnvironmentVariableValueInteger(iMax, "ARX_MaxTextureSize", 'i', "", 0, 0).getInteger();}();  // being static warns only once. TODO Experimental (still too glitchy results), so default is 0 = disabled
